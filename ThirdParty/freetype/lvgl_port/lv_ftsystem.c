...
FT_New_Memory(void)
{
    ...
    memory = (FT_Memory)lv_mem_alloc(sizeof(*memory));
    if(memory) {
        memory->user    = NULL;
         ...
         long       size)
{
    ...
    return lv_mem_alloc((size_t)size);
}
           ...
           void   *   block)
{
    ...
    return lv_mem_realloc(block, (size_t)new_size);
}
...
#endif/*lv_mem_freeTYPE_USE_LV_FTSYSTEM*/
